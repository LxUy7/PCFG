...
public:
    ...
    // vector<uint32_t> ordered_values; 
...
public:
    ...
    vector<int> curr_indices;  // 当前segment的value索引（基于ordered_values的32位整型数组）
    vector<int> max_indices;   // 每个segment的value总数（即ordered_values的大小）
    // void init();
    float preterm_prob;
    float prob;
};
...
public:
    ...
    static string uint32_to_str(uint32_t val) {
        char buf[5] = {0};
        memcpy(buf, &val, 4);
        return string(buf);
    }
